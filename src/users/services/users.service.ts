...
export class UsersService {
    ...
    public async findBy({ key, value }: { key: keyof UserCreateDto; value: any }) {
        try {
            const user: UsersEntity = await this.userRepository.createQueryBuilder('user').addSelect('user.password').leftJoinAndSelect('user.roles', 'roles').leftJoinAndSelect('user.fotografo', 'fotografo').where({ [key]: value }).andWhere(
    ...
    public async findOneAuth(id: string): Promise<UsersEntity> {
        try {
            const user: UsersEntity = await this.userRepository.createQueryBuilder('user').leftJoinAndSelect('user.roles', 'roles').leftJoinAndSelect('user.fotografo', 'fotografo').where('user.id = :id', { id }).andWhere('user.isDeleted = false').getOne();
